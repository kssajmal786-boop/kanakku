...
import { toDateOnly } from '../parsers/date.parser';
...
export interface GeminiBatchResult {
  ...
  errors: Array<{ gmailMessageId: string; subject: string; error: string }>;
 ...
 */
export async function extractTransactionWithGemini(
  email: ParsedEmail
): Promise<GeminiProcessResult> {
  const messageId = email.messageId;

  // Guard: Gemini must be available
  if (!config.gemini.isAvailable) {
    return {
      success: false,
      transaction: null,
      skipped: true,
  ...
  try {
    ...
    if (
      extraction.transactionType === 'failed' ||
    ...
    if (validationErrors.length > 0) {
      logger.warn('Gemini extraction failed validation', {
        messageId,
        senderDomain: email.senderDomain,
...
export async function extractBatchWithGemini(
  ...
  const processed: CanonicalTransaction[] = [];
  const needsReview: CanonicalTransaction[] = [];
  const errors: Array<{ gmailMessageId: string; subject: string; error: string }> = [];
  let skipped = 0;

  // If Gemini is not available, signal caller to use regex fallback
  if (!config.gemini.isAvailable) {
    logger.info('Gemini unavailable — batch will use regex parser fallback');
    return {
      processed: [],
      skipped: 0,
      needsReview: [],
  ...
  logger.info('Starting Gemini batch extraction', {
      ...
      if (!result.success || !result.transaction) {
        ...
        if (isApiError) {
          ...
          if (consecutiveApiErrors >= MAX_CONSECUTIVE_API_ERRORS) {
            logger.warn('Gemini API returning persistent errors — aborting Gemini extraction, regex fallback will handle remaining emails', {
  ...
  logger.info('Gemini batch extraction complete', {
    total: emails.length,
 ...
 */
function buildEmailContent(email: ParsedEmail): string {
  const parts: string[] = [];
 ...
 */
function parseGeminiResponse(rawText: string): GeminiExtractionResult | null {
  ...
  try {
    ...
    if (jsonMatch) {
      try {
        const parsed = JSON.parse(jsonMatch[0]) as GeminiExtractionResult;
        if (typeof parsed.isTransaction !== 'boolean') return null;
        return parsed;
      } catch {
        return null;
 ...
 */
function validateExtraction(extraction: GeminiExtractionResult): string[] {
  const errors: string[] = [];
 ...
 */
function mapToCanonicalTransaction(
  ...
  const transaction: CanonicalTransaction = {
    ...
    category,
    paymentMethod,
    source: 'gmail' as TransactionSource,
    description,
    ...
    linkedTxnId: null,
    confidence,
    parseStatus: (confidence >= 0.7 ? 'success' : confidence >= 0.4 ? 'partial' : 'failed') as ParseStatus,
    ...
    metadata: {
      ...
      parserNotes: `gemini:${config.gemini.model}`,
 ...
 */
function mapTransactionType(geminiType: string): TransactionType {
  switch (geminiType) {
    case 'income':
      return 'income';
    case 'expense':
...
function mapPaymentMethod(method?: string): PaymentMethod {
  const valid: PaymentMethod[] = ['upi', 'card', 'cash', 'bank', 'atm', 'netbanking', 'unknown'];
  if (method && valid.includes(method as PaymentMethod)) {
 ...
 */
function mapCategory(category?: string, type?: TransactionType): TransactionCategory {
 ...
 */
function buildDescription(extraction: GeminiExtractionResult, type: TransactionType): string {
  ...
  // Fallback
  if (type === 'income') return 'Income received';
  return 'Payment';
}
 ...
 */
function buildTags(extraction: GeminiExtractionResult): string[] {
  const tags: string[] = [];
  if (extraction.paymentMethod === 'upi') tags.push('upi');
  if (extraction.paymentMethod === 'card') tags.push('card');
  if (extraction.paymentMethod === 'atm') tags.push('atm', 'cash-out');
  if (extraction.transactionType === 'refund') tags.push('refund');
  if (extraction.transactionType === 'reversal') tags.push('reversal');
 ...
 */
function inferBankFromDomain(domain: string): string | undefined {
