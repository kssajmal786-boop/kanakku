...
import {
  ...
  executeWithBackoff,
  isQuotaError,
} from './gmailFetcher.service';
import { parseEmailBatch } from '../parsers/email.parser';
import { extractBatchWithGemini } from './geminiExtractor.service';
import { deduplicateBatch } from './deduplication.service';
import { validateBatch } from './normalizer.service';
import type { GmailMessage, GmailMessagePayload, ParsedEmail } from '../types/gmail.types';
...
export const FINANCIAL_SEARCH_QUERY = `(${FINANCIAL_SEARCH_KEYWORDS}) ${FINANCIAL_EXCLUSIONS}`;
 ...
 */
export function scoreCandidate(snippet: string, subject = '', from = ''): number {
  ...
  if (text.includes('debited') || text.includes('account debited') || text.includes('amount debited')) score += 10;
  if (text.includes('credited') || text.includes('account credited') || text.includes('amount credited')) score += 10;
  if (text.includes('upi payment') || text.includes('upi credit') || text.includes('paid via upi') || text.includes('received via upi')) score += 10;
  if (text.includes('card used') || text.includes('pos transaction') || text.includes('card debited')) score += 8;
  if (text.includes('neft') || text.includes('imps') || text.includes('rtgs') || text.includes('fund transfer')) score += 8;
  if (text.includes('atm withdrawal') || text.includes('cash withdrawal')) score += 10;
  if (text.includes('transaction successful') || text.includes('txn alert') || text.includes('payment successful')) score += 7;
 ...
 */
export function buildGmailDateQuery(startDate: string, endDate: string): {
  afterDate: string;
  beforeDate: string;
...
function createGmailClient(auth: OAuth2Client) {
...
export async function fetchFinancialEmails(
  ...
  const fetchBudget = options.fetchBudget ?? config.gmail.maxMessageFetches;
  const maxDiscoveryLimit = Math.min(options.maxResults ?? config.gmail.maxEmailsPerSync, 500);

  // 1. Retrieve existing sync state for user (if available)
  const existingSyncState = await db.getGmailSyncState(userId);

  // 2. Build intelligent date query
  let query = FINANCIAL_SEARCH_QUERY;
  let queryDateUsed: string | null = null;
  ...
  if (options.startDate && options.endDate) {
    ...
    lookbackLimit.setDate(lookbackLimit.getDate() - (options.lookbackDays ?? config.gmail.initialLookbackDays));
    ...
    const lookbackDays = options.lookbackDays ?? config.gmail.initialLookbackDays;
  ...
  const discoveredMessageIds: string[] = [];
  let pageToken: string | undefined;

  logger.info('GMAIL SYNC START', {
    userId,
    queryDate: queryDateUsed,
    lookbackDays: config.gmail.initialLookbackDays,
    fetchBudget,
    priorDeferred: deferredFromPrior.length,
    concurrency: config.gmail.fetchConcurrency,
  });

  try {
    do {
      const listResponse = await executeWithBackoff(
        () =>
    ...
    if (isQuotaError(err)) {
      logger.warn('Gmail search encountered quota limit during message discovery');
      return {
        messages: [],
        nextHistoryId: null,
  ...
  if (totalFound === 0) {
    logger.info('GMAIL SYNC COMPLETE: no candidate messages found');
  ...
  const alreadyProcessed = await db.getProcessedMessageIds(userId, combinedIds);
  const unprocessedIds = combinedIds.filter((id) => !alreadyProcessed.has(id));

  logger.info('GMAIL DISCOVERY CANDIDATES', {
    totalFound,
  ...
  if (unprocessedIds.length === 0) {
    logger.info('GMAIL SYNC COMPLETE: all candidates already processed');
    const profileResponse = await gmail.users.getProfile({ userId: 'me' }).catch(() => null);
    const nextHistoryId = profileResponse?.data.historyId ?? null;
  ...
  const { results: scoredCandidates, quotaExceeded: metadataQuotaHit } = await fetchQueue<string, ScoredCandidate>(
    inspectionSlice,
    async (id) => {
    ...
    {
      concurrency: config.gmail.fetchConcurrency,
      delayMs: config.gmail.fetchDelayMs,
  ...
  if (toSkip.length > 0) {
    ...
    await db.recordProcessedMessages(skipRecords);
    logger.info(`Skipped ${toSkip.length} non-transaction emails (OTPs, statements, alerts)`);
  }
  ...
  if (selectedForFetch.length > 0 && !fullFetchQuotaExceeded) {
    const queueResult = await fetchQueue<string, GmailMessage>(
      selectedForFetch,
      async (id) => {
        const resp = await gmail.users.messages.get({
          userId: 'me',
  ...
  if (fetchedMessages.length > 0) {
    ...
    await db.recordProcessedMessages(processedRecords);
  }

  // Find latest message date among fetched messages if any
  let latestMsgDate: string | null = existingSyncState?.last_successful_message_date ?? null;
  ...
  // 8. Update sync state
  const syncStatus: 'completed' | 'partial' | 'quota_exceeded' = fullFetchQuotaExceeded
  ...
  // 9. AGGREGATE LOGGING
  logger.info('GMAIL FETCH SUMMARY', {
...
// ─── Safe Metadata Retrieval (Quota-Safe) ────────────────────────────────────
...
async function fetchSafeMetadata(auth: OAuth2Client, messageId: string): Promise<SafeMessageSummary | null> {
  const gmail = createGmailClient(auth);
...
export async function listRecentMessages(
  auth: OAuth2Client,
  maxResults = 10
): Promise<SafeMessageSummary[]> {
  const ids = await listMessageIds(auth, '', maxResults);
  const queueResult = await fetchQueue<string, SafeMessageSummary>(
    ids,
    async (id) => fetchSafeMetadata(auth, id),
    {
      concurrency: config.gmail.fetchConcurrency,
      delayMs: config.gmail.fetchDelayMs,
...
// ─── AI-Controlled Date Range Transaction Extraction ─────────────────────────

export interface SearchTransactionsByDateOptions {
  userId: string;
  startDate: string; // YYYY-MM-DD
  endDate: string;   // YYYY-MM-DD
  query?: string;    // e.g. "Swiggy"
  fetchBudget?: number;
 ...
 */
export async function searchGmailTransactionsByDate(
  auth: OAuth2Client,
  options: SearchTransactionsByDateOptions
): Promise<SearchTransactionsByDateResult> {
  const {
    userId,
    startDate,
    endDate,
    query: userQuery,
    fetchBudget = config.gmail.maxMessageFetches,
  ...
  let searchQuery = `(${FINANCIAL_SEARCH_QUERY}) ${queryFragment}`;
  if (userQuery && userQuery.trim()) {
    searchQuery += ` (${userQuery.trim()})`;
  }

  logger.info('GMAIL DATE-RANGE SEARCH START', {
    userId,
    startDate,
    endDate,
    userQuery,
  ...
  try {
    const listResp = await executeWithBackoff(
      () =>
        gmail.users.messages.list({
          userId: 'me',
          q: searchQuery,
  ...
  logger.info(`Found ${candidateIds.length} candidate emails for date range ${startDate} to ${endDate}`);
  ...
  const { results: scoredCandidates } = await fetchQueue<string, ScoredItem>(
    candidateIds.slice(0, Math.min(candidateIds.length, 30)),
    async (id) => {
      const resp = await gmail.users.messages.get({
        userId: 'me',
        id,
  ...
  if (selectedIds.length > 0) {
    const queueRes = await fetchQueue<string, GmailMessage>(
      selectedIds,
      async (id) => {
        const resp = await gmail.users.messages.get({
          userId: 'me',
  ...
  let extractedTransactions: CanonicalTransaction[] = [];
  let parseFailures = 0;
  let needsReview = 0;
  let extractionMethod = 'regex';

  if (useGemini) {
    const geminiRes = await extractBatchWithGemini(parsedEmails);
    if (geminiRes.geminiUsed) {
      ...
      if (geminiRes.errors.length > 0) {
        ...
        const { parsed: fallbackParsed, failures: fallbackFailures } = parseEmailBatch(errEmails);
        extractedTransactions.push(...fallbackParsed);
        parseFailures = fallbackFailures.length;
        if (geminiRes.processed.length > 0) extractionMethod = 'gemini+regex';
      }
    } else {
  ...
  const { valid } = validateBatch(extractedTransactions);
  const { unique, duplicatesSkipped } = deduplicateBatch(valid, existingTransactions);
  ...
  logger.info('GMAIL DATE-RANGE SEARCH COMPLETE', {
    startDate,
    endDate,
    emailsFound: candidateIds.length,
