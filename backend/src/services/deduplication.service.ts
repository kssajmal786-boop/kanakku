...
function stringSimilarity(a: string, b: string): number {
  ...
  if (longer.includes(shorter)) return 0.9; // One is a substring of the other
...
function scoreTransactionPair(
  ...
  const incomingDay = toDateOnly(incoming.date);
...
export function deduplicateBatch(
  ...
  duplicateDetails: Array<{ incomingId: string; reason: string; matchedId: string | null }>;
} {
  const accepted: CanonicalTransaction[] = [];
  const duplicateDetails: Array<{ incomingId: string; reason: string; matchedId: string | null }> = [];
