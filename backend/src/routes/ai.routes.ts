...
interface ChatRequest {
  ...
  financialContext: {
    ...
    period?: {
      ...
      largestIncome: { amount: number; description: string; date: string } | null;
    };
    healthScore?: {
      score: number;
...
function extractAction(text: string): { cleanText: string; action: unknown | null } {
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const { message, financialContext, history = [], language = 'en' } =
      req.body as ChatRequest;

    if (!message || typeof message !== 'string' || message.trim().length === 0) {
      sendError(res, 'Message is required', 400, 'MISSING_MESSAGE');
      return;
    }

    if (message.length > 2000) {
      sendError(res, 'Message too long (max 2000 characters)', 400, 'MESSAGE_TOO_LONG');
      return;
    }
    ...
    if (
      ...
      sendSuccess(res, { reply: refusal, action: null, fallback: false });
      return;
    }

    // Graceful fallback if Gemini API key is not configured or invalid
    if (!config.gemini.isAvailable) {
      logger.info('Gemini API key not configured — using deterministic rule engine');
    ...
    try {
      ...
      logger.info('AI chat processed successfully', {
      ...
      logger.error('Gemini AI call error', { error: error.message, status: error.status });
...
function buildContextString(ctx: ChatRequest['financialContext'], lang: string): string {
  ...
  if (ctx.today) {
    ...
    lines.push(
      `Today's Summary:`,
      `  Income: ${curr}${formatINR(d.totalIncome / 100)}`,
      `  Expenses: ${curr}${formatINR(d.totalExpenses / 100)}`,
      `  Net Cash Flow: ${d.netCashFlow >= 0 ? '+' : ''}${curr}${formatINR(d.netCashFlow / 100)}`,
  ...
  if (ctx.period) {
    const p = ctx.period;
    lines.push(
      `Period Summary (${p.label}):`,
      `  From: ${p.startDate} To: ${p.endDate}`,
      `  Total Income: ${curr}${formatINR(p.totalIncome / 100)}`,
      `  Total Expenses: ${curr}${formatINR(p.totalExpenses / 100)}`,
      `  Net: ${p.netCashFlow >= 0 ? '+' : ''}${curr}${formatINR(p.netCashFlow / 100)}`,
      `  Transactions: ${p.transactionCount}`,
      `  Daily average expense: ${curr}${formatINR(p.avgDailyExpense / 100)}`
    );
...
function generateFallbackResponse(
  message: string,
  ctx: ChatRequest['financialContext'],
  lang: string,
  history: ChatMessage[] = []
): { reply: string; action: unknown | null } {
  ...
  const askedForCategory =
    lastAiMsg.includes('what did you spend') ||
