 ...
 */
router.post('/validate', validation_middleware_1.validateTransactionInput, (req, res) => {
    const { transactions } = req.body;
    const { valid, invalidCount } = (0, normalizer_service_1.validateBatch)(transactions);
    logger_1.logger.info('Transaction validation complete', {
...
router.post('/normalize', async (req, res) => {
    ...
    if (!transaction || typeof transaction !== 'object') {
        (0, response_1.sendError)(res, 'transaction object is required', 400, 'MISSING_TRANSACTION');
        return;
    }
    const result = (0, normalizer_service_1.normalizeAndValidate)(transaction);
    if (result.valid) {
        (0, response_1.sendSuccess)(res, { transaction: result.transaction });
    }
    else {
        res.status(422).json({
            success: false,
...
router.post('/deduplicate', async (req, res) => {
    ...
    if (!incoming || typeof incoming !== 'object') {
        (0, response_1.sendError)(res, 'incoming transaction is required', 400, 'MISSING_INCOMING');
        return;
    }
    const result = (0, normalizer_service_1.normalizeAndValidate)(incoming);
    if (!result.valid) {
        (0, response_1.sendError)(res, 'Incoming transaction is invalid', 422, 'INVALID_TRANSACTION');
        return;
    }
    const dedupResult = (0, deduplication_service_1.checkDuplicate)(result.transaction, existing);
    (0, response_1.sendSuccess)(res, {
 ...
 */
router.post('/manual', validation_middleware_1.validateManualTransaction, (req, res) => {
    const user = req.user;
    const input = req.body;
    const raw = (0, normalizer_service_1.buildManualTransaction)(input, user.userId);
    const result = (0, normalizer_service_1.normalizeAndValidate)(raw);
    if (result.valid) {
        logger_1.logger.info('Manual transaction built', { userId: user.userId });
        (0, response_1.sendSuccess)(res, { transaction: result.transaction }, 201);
    }
    else {
        res.status(422).json({
            success: false,
