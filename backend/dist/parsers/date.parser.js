...
exports.isWithinLookback = isWithinLookback;
const MONTH_MAP = {
...
const DATE_PATTERNS = [
    // ISO: 2024-08-17 (with optional time)
    {
        ...
        parse: (m) => {
            const d = new Date(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3]), parseInt(m[4] ?? '0'), parseInt(m[5] ?? '0'), parseInt(m[6] ?? '0'));
    ...
    // DD/MM/YYYY or DD-MM-YYYY (with optional time HH:MM:SS)
    {
        ...
        parse: (m) => {
            const d = new Date(parseInt(m[3]), parseInt(m[2]) - 1, parseInt(m[1]), parseInt(m[4] ?? '0'), parseInt(m[5] ?? '0'), parseInt(m[6] ?? '0'));
            return isValidDate(d) ? d : null;
        },
    },
 ...
 */
function isWithinLookback(dateStr, lookbackDays) {
